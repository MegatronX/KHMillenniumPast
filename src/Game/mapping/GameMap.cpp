...
namespace Game
{
	namespace Mapping
	{
		...
		GameMap::GameMap(const std::string &file, const std::string &name, Engine* engine) : TileMap(file, name, engine), UsePythonMove(false), UsePythonScriptClass(false), EncounterRate(0.2f), rng(time(0)), RandomFloatGen(rng, boost::uniform_real<float>(0, 1)), Entity(name, TileMapEntity), ActiveExit(nullptr)
		{
