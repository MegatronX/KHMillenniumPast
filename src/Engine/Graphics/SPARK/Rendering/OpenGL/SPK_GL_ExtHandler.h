...
#ifndef H_SPK_GL_EXTHANDLER
#define H_SPK_GL_EXTHANDLER
...
namespace GL
{
	...
	* <li>point sprites : used in SPARK to attach images on points and therefore gain performance</li>
	...
	*/
	...
	inline void GLExtHandler::setPixelPerUnit(float fovy,int screenHeight)
	{
	...
	inline void GLExtHandler::glTexImage3D(GLenum target,
			GLint level,
			GLenum internalFormat,
			GLsizei width,
			GLsizei height,
	...
	inline GLExtHandler::GLExtension GLExtHandler::getPointSpriteGLExt()
	{
		return pointSpriteGLExt;
	}

	inline GLExtHandler::GLExtension GLExtHandler::getPointParameterGLExt()
	{
		return pointParameterGLExt;
	}

	inline void GLExtHandler::disablePointParameterGLExt()
	{
		SPK_glPointParameterfv(GL_POINT_DISTANCE_ATTENUATION,QUADRATIC_SCREEN);
	}

	inline GLExtHandler::GLExtension GLExtHandler::getTexture3DGLExt()
	{
		return texture3DGLExt;
	}

	inline GLExtHandler::GLExtension GLExtHandler::getShaderGLExt()
	{
		return shaderGLExt;
	}
}}
