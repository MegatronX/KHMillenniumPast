...
class  Timer
{
	...
	public:
		...
		/// @param timeLimit The initial time, in milliseconds (must be greater than zero).
		/// @param initiallyRunning Indicates whether the timer is immediately being started.
		explicit					Timer(sf::Uint32 timeLimit, bool initiallyRunning = false);
		
		/// @brief Returns the remaining time in milliseconds.
		/// @details If the timer has expired, 0 is returned.
		sf::Uint32					GetRemainingTime() const;
		...
		/// @param timeLimit The new time, in milliseconds (must be greater than zero).
		/// @param continueRunning Indicates whether the timer directly continues.
		virtual void				Reset(sf::Uint32 timeLimit, bool continueRunning = false);
